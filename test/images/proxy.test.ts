...
test('strips query string', () => {
  expect(normalizeUrl('https://x/a.webp.jpg?v=1', resolveOptions()).url).toBe(
    'https://x/a.webp.jpg',
  );
});
...
test('proxy rewrites image url with normalized url', () => {
  ...
  const r = rewriteImages(
    {
      messages: [
        { message: { attachment: { type: 'image', payload: { url: 'https://x/a.jpg?v=1' } } } },
      ],
    },
    o,
  );
...
test('detect-only without proxy removes unverifiable webp image', () => {
  ...
  const r = rewriteImages(
    {
      messages: [
        { message: { attachment: { type: 'image', payload: { url: 'https://x/a.webp' } } } },
      ],
    },
    o,
  );
  expect(
    (r.payload.messages![0] as { message: { attachment?: unknown } }).message.attachment,
  ).toBeUndefined();
  expect(r.diagnostics.some((d) => d.code === 'PENDING_IMAGE_UNVERIFIABLE')).toBe(true);
});

test('proxyUrl encodes the target url', () => {
  expect(proxyUrl('https://p', 'https://x/a b.jpg')).toBe(
    'https://p?u=' + encodeURIComponent('https://x/a b.jpg'),
  );
});
